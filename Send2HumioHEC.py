...
class Send_to_HEC():
    ...
    def send_to_HEC(event_data, num_devices):
        ...
        try:
            ...
            if proxy_used == False:
                r = requests.post(url=HumioHECurl, headers=header, data=event_data, verify=HumioHECverify, timeout=300)
                transmit_result = r.status_code
            elif proxy_used == True:
                r = requests.post(url=HumioHECurl, headers=header, data=event_data, verify=HumioHECverify, timeout=300, proxies=proxies)
                transmit_result = r.status_code

            
            logging.info('Devices2Humio v' + version + ':  HEC: Transmission status code for data push to HEC= '+ str(transmit_result))
            logging.debug('Devices2Humio v' + version + ':  HEC: Transmission device data for ' + str(num_devices) + ' to Humio HEC has successfully completed.')

        except requests.exceptions.RequestException as e:
            error=str(e)
            logging.info('Devices2Humio v' + version + ':  HEC: Unable to evaluate and transmit sensor_data event: Error: ' + error)
            try:
                sys.exit('Devices2Humio v' + version + ': HEC: This is fatal error, please review and correct the issue - CrowdStrike Intel Indicators to Humio is shutting down')
            except:
                pass
